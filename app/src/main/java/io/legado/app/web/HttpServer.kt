...
import io.legado.app.api.controller.ReadTimeController
    ...
    private fun handlePost(session: IHTTPSession, uri: String): ReturnData? {
        ...
        return runBlocking {
            when (uri) {
                ...
                "/saveBookProgress"  -> BookController.saveBookProgress(
                    postData,
                    session.parameters["flush"]?.firstOrNull().toBoolean()
                )
                "/syncBookProgress"  -> BookController.syncBookProgress(postData)
                ...
                "/saveReadTime"      -> ReadTimeController.saveReadTime(postData)
