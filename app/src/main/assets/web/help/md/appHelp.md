...
* 打开“**我的**”——“**书源管理**”；
* 长按进入选择模式，勾选要分享的书源；
* 打开选择操作菜单，点击“**分享选中源**”直接调用系统分享，或点击“**导出所选**”生成文件后分享；
* 接收方在书源管理中使用“**本地导入**”导入收到的 JSON/TXT 文件。

**【注】**
1. *完整备份是按日期命名的 ZIP，不能再按旧教程查找 `YueDu3.0/myBookSource.json`。*
2. *Android 11 及以上系统可能限制直接访问其他应用的 `Android/data` 目录，优先使用系统分享和系统文件选择器。*
...
### 1. 目前阅读支持哪些格式的本地书籍？
目前支持 TXT、EPUB、UMD、PDF、MOBI、AZW/AZW3 和 CBZ 格式，也可从 ZIP、RAR、7Z 容器中导入其中的受支持书籍。PDF 由系统渲染为图片页；部分格式的排版和交互能力会与 TXT/EPUB 不同。
...
如果尝试所有规则均无法识别，请在[当前项目 GitHub 仓库](https://github.com/hupohupochuan/legado/issues)提交 Issue，并在确认文件可公开的前提下提供最小复现样本；旧帮助中的反馈邮箱本轮无法从当前代码确认，已移除。
...
以下以坚果云的历史界面为配置示例。第三方 WebDAV 服务的套餐、配额和页面可能变化，请以服务商当前说明为准；本项目不对特定服务作可用性保证。
