...
- 记录日期: 2026-07-28
- 验证日期: 2026-07-28（缓存进度与导出计划 10 项定向 JVM 测试、Debug Kotlin 编译及 `scripts/check-debug.sh` 通过；真机长任务和实际 TXT/EPUB 文件待安装包回归）
...
- 缓存自然结束后保留可点击的完成通知，返回缓存页后继续使用每本书已有的导出入口；主动取消不生成“缓存完成”通知。
...
- 每次从缓存页导出明确选择“原始缓存正文”或“应用替换净化规则”，默认原始正文。这里的原始正文是书源规则解析后的落盘内容，不是网页原始 HTML；仍保留章节标题整理和合法 TXT/EPUB 结构。
