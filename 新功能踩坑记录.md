...
**优化约束**:
- Web 端普通进度保存使用 latest-wins 单工作协程：同一轮更新只保留最新位置，已有请求未结束时后续位置排队串行发送；相同位置不重复排队。
- 缓存跨章不再先保存安全哨兵位置，等待新 `BookPageReader` 上报真实页位置后立即保存，避免安全位置和真实位置并发上传。
- Beacon 仍在页面隐藏/离开时立即发送；手机端 `BookController` 对进度保存加互斥，覆盖 Beacon 与普通请求交错，避免同一 WebDAV 文件并发写入。
...
- 页面隐藏时 Beacon 仍能尽力保存最近进度；与尚未结束的普通保存交错时由后端串行处理。
