...
- 记录日期: 2026-07-15
- 验证日期: 2026-07-15（18 项定向 JVM 测试、Debug Kotlin 编译、Debug 检查、Web 类型检查、生产构建、APK assets 同步及 Release/R8 构建通过）。
...
- 本地非 WebDAV 书串行搜索全部目录章节；在线书和 WebDAV 书只读取一次 `BookHelp.getChapterFiles(book)` 快照，再按章节缓存文件名精确过滤。搜索服务只调用 `BookHelp.getContent`，不得依赖或调用 `WebBook.getContentAwait`。
...
- 点击结果才提交新阅读位置；目标章节正文加载成功前延后更新浏览器进度，失败时恢复原章节、位置和滚动状态。无限滚动追加在途时禁止并发跳转，避免旧追加响应混入目标章节。

**回归测试清单**:
- 本地 TXT/EPUB 能搜索全部章节；在线书和 WebDAV 书只命中已有缓存，搜索前后缓存文件数与网络请求数不增加。
- 结果分批出现且进度单调增加；停止、快速连续重搜、关闭面板、断开手机 Web 服务和路由离开后旧结果不得继续到达。
- 空关键词、超过 100 字符的关键词、无可搜索缓存、无匹配结果和超过 500 条结果分别显示明确状态。
- 滚动模式点击结果定位到目标段落，书本模式定位到包含该段落位置的书页；仅搜索或跳转失败不改变原阅读进度。
