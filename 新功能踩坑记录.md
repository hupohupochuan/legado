...
>
> 当前代码复核（2026-07-15）：本文按时间保留功能演进记录，旧段落中的“当前”可能已被后续小节替代。现行 Web 进度链路是“浏览器固定 5 秒窗口提交手机 Room，手机固定 60 秒窗口上传 WebDAV”；仅同步/恢复 WebDAV 进度使用 `RangeOnly`，不检查本地 SAF 文件可读性；Web 纸质书效果已演进为 `BookPageReader.vue` 内的固定 `clip-path` 裁切动画。未在本轮重新执行的历史真机/浏览器手测仍以原验证日期为准。
