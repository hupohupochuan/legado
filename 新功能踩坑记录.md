...
- 点击结果才提交新阅读位置；目标章节正文加载成功前延后更新浏览器进度，失败时恢复原章节、位置和滚动状态。书本模式重挂旧章节时必须把请求前的当前 `chapterPos` 作为 `initialPos`，不能复用只在切章时更新的陈旧 `bookInitialPos`，否则会退回旧页并覆盖已恢复进度。无限滚动追加在途时禁止并发跳转，避免旧追加响应混入目标章节。
...
- 滚动模式点击结果定位到目标段落，书本模式定位到包含该段落位置的书页；在书本模式中间页模拟目标章加载失败后，必须仍停在原章节原页且手机端进度不变。
