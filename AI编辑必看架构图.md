...
> 最新修订: 2026-07-31 CST
> 最近三次修订:
> - 2026-07-31 CST: 前台服务销毁必须 `stopForeground(STOP_FOREGROUND_REMOVE)` 并设 `isStopping` 门闩短路回调与重启，不得依赖系统自动清理通知。
...
- 前台服务（WebService 等）销毁时必须显式 `ServiceCompat.stopForeground(...STOP_FOREGROUND_REMOVE)` 并 `notificationManager.cancel(id)` 移除通知，同时设置 `isStopping` 门闩短路所有网络回调、后台重启检查和通知刷新；不得依赖系统自动清理前台通知，也不得在停机后再 `notify()`。
